...
| 试点装置修复 | judge ①层故障特征词 + sweep 脚本 bash3.2 兼容 | ✅ | 试点批（2026-08-31，授权①）发现：①层纯资源名匹配在 crashloop 类场景系统性低估（结论述故障机制不复述资源名，实测 1/10）——ground_truth 增可选 fault_signature，①hit = 资源名 OR 特征词任一命中，已落盘记录离线重判免重跑（试点 20 单元重判 18/20，完成单元 18/18）；两 sweep 脚本汇总行 `$skipped）` 变量紧跟全角字符在 macOS bash 3.2 + set -u 下误报 unbound（#133 同族），花括号修复；另记录：LLM 瞬态失败率 5%（重试耗尽型），裁决按纪律接受不剔除 |
...
| map-reduce-1 | 大表全覆盖 map-reduce 纯函数核心 | ✅ | PR #155（feat/0.1.4-map-reduce-core，2026-09-21 设计评审冻结后实现）；`summary` 新增 `mapreduce.go`：Pass 1 全局基准（频次 + 稀有论域 + 全局 T²）→ 等行数分片（`chooseShardCount` 候选循环精确记账动态定 S，无魔法常数）→ 片内两阶段选择（稀有代表优先 + T²/步长补位）→ Reduce 归并 + 溢出显式标注；`method` 枚举 `map-reduce` 显式启用（默认 fast 逐字节不变，G4）；行号 0 基贯通 evidence.read；产物总 rune ≤ 预算口径；config/k8s/编排零改（config 字符串经装配层 `ParseMethod` 自动放行，教学面归步骤 2）；实现期修正三处：行预算按渲染整行口径（#i 前缀含，防系统性超支）、尾注/溢出悲观预留入片节记账、稀有论域列规则不继承 0.999 均匀性阈（超均匀列极稀有值深误杀，补钉板回归）；随 PR #157 回归 main |
| map-reduce-2 | 教学面接线（Spec / tower prompt / example.yaml） | ✅ | PR #158（feat/0.1.4-map-reduce-wiring，2026-09-22）；k8s Spec 分片全覆盖读法（形态识别 + 0 基行号直达 + 溢出照参数跟进）+ tower prompt 大表导航扩分片形态读法（报数/溢出 = 存在性信号，片头区间直接作 evidence.read 参数）+ example.yaml 值清单与 budget 总产物口径（顺带补漏列的 llm-rerank）+ 教学契约锚测试 ×2；裁决：fast/greedy 渲染尾部提示不做（G4 逐字节不变；method 为启动期开关模型无切换能力）；零行为改动，接线管道步骤 1 已全通 |
| map-reduce-3 | 对照装置（rich-value fleet 生成 + 三档判分） | ✅ | PR #159（feat/0.1.4-map-reduce-bench，2026-09-24）；生成器 fleet 模式（`TableSpec.RootFleetPer100`：CrashLoopBackOff×K 多载体中权值根因 + RESTARTS/NODE 论域随 N 扩张，单遍名额挤兑随 N 显现；唯一模式逐字节不变）+ 判分三档 `ProjectionPresence`（存在性）/ `ProjectionZoneHit`（可达性：行本人或所在片报数地址，G2 机械兑底）+ map-reduce 机械臂注册（默认矩阵 2700）+ CSV 尾列 presence/zone/root_fleet + 专项矩阵 `scripts/bench-mapreduce.yaml`；实测完成标志 ② 形态：单遍 1/0/0 随 N 断崖 vs map-reduce 1/1/0 全 N 平坦；两轮探针定稿（最小版与朴素 fleet 均被实证否定，设计修订 v2）；装置期事实：片内值清单被 RareListMax=8 截断时中权值不进清单但片统计全量含它 |
| map-reduce-4 | 真场景 smoke（kind 超大表 + 真 LLM） | ✅ | PR #161（feat/0.1.4-map-reduce-smoke，2026-09-24）；bigtable-fleet 场景（CRD 大表 3000 行 + CLBO×30 散布，论域扩张常数对齐 bench rich-value fleet；gen.sh 确定性生成器）+ chat-env 场景级环境注入（lab-chat / smoke-all 自动 source；本场景 ARUING_TOOLS_PROJECTION_METHOD=map-reduce，无文件行为不变）+ scenario-up CRD discovery 竞态重试 + 稳态探针 Pod；零 Go 改动；真 LLM smoke 判定通过（4.5/5 应出现 0/3 不应：存在性精确 30 / 根因坏 tag supported 引证据 / 无害扰动诚实拒判；fast 对照臂观察：jsonpath 窄化逃生门印证 arc 脊柱 4 + `-o json` 3.3MB 触发 spill 首次自然真路径）；装置期事实（AGE 稀释 → CLBO 不进片内值清单）与位置概括改进候选记录步骤 plan |
...
**下一项**：**streaming 收线进 main**（0.1.4 第三个功能，随版交付裁决 2026-09-24）：分支 `feature/stream-out`（S1–S4 已并入：#148/#152/#154/#156，llm token 流 → 角色流式 → Turn 返 stream → TUI 增量渲染）经集成 PR 回归 main（参照 persistence #157 先例），收线后按 streaming 功能头完成标志验证（S1–S4 全链真会话逐字渲染 / 流式中断无半截消息 / 真 LLM 正负路径 smoke）并关闭功能。随后版本收尾：map-reduce 已完成（#155 / #158 / #159 / #161，三标志全绿关闭 2026-09-24，功能历程已蒸馏笔记 `历程/0.1.4/`）；版本级集成验证 `make check` + `make smoke-all` 场景全绿（含新场景 bigtable-fleet，chat-env 自动走 map-reduce 臂；裁决 2026-09-18：smoke-all 不再作欠账项）→ 打 tag `v0.1.4` 发布（production 流程）。

**候选方向**（远景与排序依据见笔记 `plan/version/0.2.0.md`；遗留清单见笔记 `plan/archive/0.2.0/0.1.3/2026-8-31-open-issues.md`；0.1.4 已立项三项不再列此处）：

1. **论文期实验扩点**：断崖曲线 100/200 轮（20/50 两点不可判的钉板结论）；synthesis 类探针短板的产品侧跟进
