...
**下一项**：**0.1.4 map-reduce 步骤 3（对照数据）**：bench 扩超大表维度（数千行 × 根因位置头/中/尾 × fast / greedy / map-reduce → CSV，零 LLM 零集群；完成标志 ②；开放问题见 requirements §7-6 场景造法）；步骤 1–2 已完成（步骤 1 #155 随 #157 进 main；步骤 2 #158 教学面，裁决 fast/greedy 尾部提示不做守 G4）；步骤 4（kind 超大表真 LLM smoke，完成标志 ①）随后。**并行裁决项**：streaming（分支 feature/stream-out，S1–S4 已并入该分支 #148/#152/#154/#156，未进 main）是否随 0.1.4 交付由维护者裁决；版本级集成验证（`make check` + `make smoke-all`）随收尾例行执行（裁决 2026-09-18：smoke-all 不再作欠账项）后打 tag 发布。
