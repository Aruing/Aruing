...
| beta5-fix-2 T-shallow-2 | 基线轻量 recon 注入 | ⏳ #51 | 每 Turn ≤1 次 api-resources → `cluster_resources`；同源 parse |
...
**下一项**：**`0.1.0-beta5-fix-2` T-shallow-1 + T-shallow-3**（证据纪律 prompt + 配额从宽/触顶 escalate）→ 合 #51 后 smoke 收尾（T-shallow-4）。设计见笔记 `plan/0.1.0-beta5-fix-2/`。须守 #2/#15–#18。
...
6. **beta5-fix-2（进行中）**：T-shallow-2 recon 注入（#51）；本步证据纪律 + 默认 12 轮/触顶 escalate
