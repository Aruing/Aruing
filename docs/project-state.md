...
**下一项**：**0.1.4 map-reduce 步骤 4（真场景 smoke，完成标志 ①）**：kind 超大表场景（根因藏任意位置）+ 真 LLM 正路径 smoke（场景造法开放问题见 requirements §7-6，fleet 连续块与组合异常形态为已识别拓展点）；步骤 1–3 已完成（#155 纯函数核心 / #158 教学面 / #159 对照装置：完成标志 ② 已钉——单遍「有存在无地址无本人行」随 N 断崖 vs map-reduce「有存在有地址无本人行」全 N 平坦）。**并行裁决项**：streaming（分支 feature/stream-out，S1–S4 已并入该分支 #148/#152/#154/#156，未进 main）是否随 0.1.4 交付由维护者裁决；版本级集成验证（`make check` + `make smoke-all`）随收尾例行执行（裁决 2026-09-18：smoke-all 不再作欠账项）后打 tag 发布。
