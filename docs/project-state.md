...
> 最后更新：2026-08-29（**`0.1.1` 已发布**：v0.1.1 tag，代表性投影 + 评测基建（#112/#114/#116）；下一里程碑 `0.1.2` 主动取证决策。0.2.0 远景与排序依据见笔记 `plan/version/0.2.0.md`）
...
**下一项**：`0.1.2`（主动取证决策 / 创新点一）设计起草——新包 `internal/agent/acquire`（信念状态 + 贝叶斯更新对数域防下溢、判别矩阵解析、EIG + 成本归一动作选择、MSPRT 三出口、假设空间活性）、Planner 输出结构与编排接线随步骤 plan 冻结后实现；动作仍全经 Dispatcher（#16），core 数据结构零改优先。算法依据在笔记仓思考文档（§1–§7、§10）。开工前可选：C4 / 下游诊断 LLM 端到端矩阵授权跑数（0.1.1 实验完整性收尾）。排序依据见笔记 `plan/version/0.2.0.md`。
