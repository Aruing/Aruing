...
> 最后更新：2026-09-20（streaming S1/S2 已合并，下一项 S3）
...
**当前实现**：0.1.4 streaming S1 已由 #148 合并，提供同步增量回调、正常结束判定、失败/未知用量统计和 Reporter 私有 JSON 收集（断流或空响应最多退避重试 2 次，失败片段不提交）。S2 已由 #152 合并，将 Tower 拆成结构化路由与最终 reply 生成；阻塞和流式 reply 使用同一上下文与提示，仅 reply 对外发增量，Tool、澄清和诊断结果仍完整提交。TUI 仍整块显示。

**下一项**：推进 S3，让 `session.Service.Turn` 暴露流式消费入口并保持失败时不落半截助手消息；Tool 只经 Dispatcher（#15–#17）。步骤设计见笔记 `plan/0.1.4/streaming/`。
