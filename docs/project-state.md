...
> 最后更新：2026-09-19（streaming S1 收集重试修复，尚未合并）
...
**当前实现**：0.1.4 streaming S1 提供同步增量回调、正常结束判定、失败/未知用量统计和 Reporter 私有 JSON 收集（断流或空响应最多退避重试 2 次，失败片段不提交）；保留原阻塞接口，尚未合并。TUI 仍整块显示。

**下一项**：S1 验证与合并后推进 S2，补齐 Tower/Responder 的正文路径；结构化领域对象仍完整校验后提交，Tool 只经 Dispatcher（#15–#17）。步骤设计见笔记 `plan/0.1.4/streaming/`。
