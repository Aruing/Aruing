...
> 最后更新：2026-09-21（streaming S1–S3 已合并，S4 终端增量展示实现与验证）
> 发布基线：2026-09-05（**v0.1.3 已发布**——收尾 docs #139/#140 → production PR #141 → tag → publish 全链走完，release notes 含 0.1.2 全部增量；#142 pr-agent #141 评审跟进修复。实验数据与钉板记录在笔记仓 `gproject/中期/实验数据/0.1.2-0.1.3-统一批/`，遗留问题见笔记仓 `plan/archive/0.2.0/0.1.3/2026-8-31-open-issues.md`）

## 当前阶段

**`0.2.0` / 完备不脆弱的证据型诊断助手：🚧 三创新点版本全部交付**（远景 2026-08-21 定稿，笔记 `plan/version/0.2.0.md`）。定位：对标 codex / opencode 的工程完备度（不中断、不断崖、不截断）+ 毕业论文三创新点载体（动机与排序依据在笔记仓）。**版本节奏**：小版本递增，每个 0.1.x = 一个可用正式实现：`0.1.1`（创新点二·代表性投影）✅ v0.1.1 已发布；`0.1.2`（创新点一·主动取证决策）✅ done 2026-08-31；`0.1.3`（创新点三·分层记忆）✅ done 2026-08-31，随 **v0.1.3 单 tag** 一并发布（0.1.2 不独立发版，发布裁决 2026-08-30）；`0.1.4`（产品完备批：持久化 + 流式 + map-reduce）🚧 已立项 2026-09-10（plan 见笔记 `plan/0.1.4/`；S1 流式运输已由 #148 合并；S2 Tower reply 流已由 #152 合并；S3 会话流式入口已由 #154 合并；S4 两种 TUI 增量展示已实现、待合并）。三创新点均已「形式化 + 实现 + 同装置对比数据」齐备，实验数据归档笔记仓 `gproject/中期/实验数据/`（0.1.1 主实验 + 0.1.2-0.1.3 统一实验批含钉板 README）；plan 已归档 `plan/archive/0.2.0/{0.1.2,0.1.3}/`。
...
**当前实现**：0.1.4 streaming S1–S3 已由 #148/#152/#154 合并。S4 的 inline / app 均消费 `TurnStream`，首段正文到达后停止等待指示，增量按主题和 Markdown 渲染，完整结果只显示一次。生成中 Ctrl+C 取消本轮，错误明确显示并可继续下一轮；增量不作为业务消息提交。相关测试、全仓 race/shuffle 测试与静态检查通过；本地 SSE 服务配真 PTY 的两种界面均通过增量、取消、续聊与退出验证。真实模型验收暂受本地凭据 401 阻碍，真集群 smoke 受 Docker daemon 未运行阻碍。

**下一项**：完成 S4 评审、真 LLM 正/负路径与真集群 smoke 验证；合并后进行流式与持久化的联合验证。Tool 只经 Dispatcher（#15–#17），TUI 保持纯展示层（#20）。步骤设计见笔记 `plan/0.1.4/streaming/`。
