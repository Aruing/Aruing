...
**下一项**：**0.1.4 persistence 步骤 4：`aruing sessions`**（`ListSessions` 读取能力 + CLI 列表命令：id / 时间 / 消息数 / 首问预览，功能头标志 5）；设计待写。后续：streaming / map-reduce 并行排期由维护者裁决。注：步骤 1/2/3 合并前 smoke 均未单独跑——裁决 2026-09-16 随版本收尾 `make smoke-all` 一并兜底。
