...
**下一项**：**0.1.4 收尾**：map-reduce 步骤 1–4 全部完成（#155 / #158 / #159 / #161，功能三标志全绿：① 真场景真 LLM 不漏 / ② bench 机械对照断崖 vs 平坦 / ③ 默认路径逐字节不变）。剩余三件：① streaming 裁决（分支 feature/stream-out，S1–S4 已并入该分支 #148/#152/#154/#156，未进 main）是否随版由维护者裁决；② 版本级集成验证 `make check` + `make smoke-all` 场景全绿（含新场景 bigtable-fleet，chat-env 自动走 map-reduce 臂）随收尾例行执行（裁决 2026-09-18：smoke-all 不再作欠账项）；③ 打 tag `v0.1.4` 发布（production 流程）。
