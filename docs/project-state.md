...
> 最后更新：2026-09-19（streaming S1/S2 已合并，S3 会话流式入口实现与验证）
...
**当前实现**：0.1.4 streaming S1/S2 已由 #148/#152 合并。S3 新增 `Service.TurnStream` 同步增量消费入口，与阻塞 `Turn` 共用提交路径；流式应答成功后才写 checkpoint 和完整助手消息，消费者错误取消上游，上游失败或取消不写应答产物。缺少流式能力或消费者明确失败。TUI 仍整块显示。

**下一项**：完成 S3 评审与合并后推进 S4，TUI 消费增量并渲染 streaming buffer；Tool 只经 Dispatcher（#15–#17），TUI 保持纯展示层（#20）。步骤设计见笔记 `plan/0.1.4/streaming/`。
