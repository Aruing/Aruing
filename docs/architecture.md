...
19. **工具输出为可导航双结构，不是字符串**：`Evidence.Raw` 是工具产出的不可变原带（产出后只留存、不回写）；`Summary` 与后续派生视图是投影。工具只做**机械格式投影**（解析已知输出格式、提取行/列/状态字段），**不做业务判断**（判断归模型 + 编排）。模型经 `Summary` 看全貌、按需 narrow 查询或 drill 到 `Raw` 切片；不得用整块文本或截断预览逼模型猜全貌。分片汇总（map-reduce）类全覆盖扫描作为专门能力存在，不是工具输出默认出口。详见笔记 `plan/arc/tool-output-navigation.md`
