...
**`0.1.1` —— 代表性投影 + 评测基建。** 大工具输出在 token 预算下经加权贪心代表性选择投影（覆盖 + 异常目标；头尾 / 均匀 / 全量基线同一二进制经配置切换），并交付评测栈：场景真值四元组、逐 run 评测记录（`aruing run --eval-json`）、机械判分 CLI、可复现投影基准 `make bench`。0.1.0 的能力全部保留：真集群 + 真 LLM 诊断、交互式终端对话、证据导航、澄清挂起与恢复、一行安装与自更新。
