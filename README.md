...
版本 `0.1.0` / 可追问的诊断助手 进行中；当前活跃里程碑 **`0.1.0-beta6`（按 run 深解）**。默认经 Tower 智能基线（含 `cluster_resources`），需要根因时升格诊断管道；L0–L2 compact + checkpoint；escalate 成功后进程内 `RunLedger` 可按 `RunID` 读回，同会话追问注入 `prior_run_details`（结论+证据）。`aruing run` 单轮诊断；`aruing chat` 多轮会话（须配置 LLM）。详见 `docs/project-state.md`。
