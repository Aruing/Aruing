...
// 大表阈值：行数超过该值时 Summary 只展示头尾若干行并引导收窄，避免把超长输出塞进上下文
// 全量行始终保留在 Raw，不静默丢行（#18）
const (
...
// 超过大表阈值时只展示头尾若干行并提示收窄或查 Raw，不静默丢行（#18）
func renderTable(label string, columns []string, rows [][]string, hasMore bool) string {
	...
	switch {
	case len(rows) > largeTableThreshold:
		fmt.Fprintf(&b, "  （仅展示前 %d + 后 %d 行；用 --field-selector / -o jsonpath 收窄，或查 raw 看全部）\n", largeTableHead, largeTableTail)
		writeRows(&b, rows[:largeTableHead])
		b.WriteString("  …\n")
		writeRows(&b, rows[len(rows)-largeTableTail:])
	default:
		writeRows(&b, rows)
	}
