...
type Query struct {
	// 问题结构编号，格式为 query_ + UUIDv7，创建时生成
	ID string `json:"id"`
...
type Node struct {
	// 节点编号，格式为 node_ + UUIDv7，供关系和后续任务引用
	ID string `json:"id"`
	// 开放的节点类型，例如 resource 或 symptom，未知类型也可以原样保存
...
type Edge struct {
	// 关系编号，格式为 edge_ + UUIDv7，供任务和证据准确引用
	ID string `json:"id"`
	// 起点节点编号，表示关系从哪个对象出发
	From string `json:"from"`
	// 终点节点编号，表示关系指向哪个对象
	To string `json:"to"`
	// 开放的关系类型，例如 calls 或 depends_on，不限制为预定义集合
	Type string `json:"type,omitempty"`
