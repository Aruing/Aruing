...
//
// 所有实体的编号字段采用「前缀加时间有序全局唯一标识」格式，例如运行编号前缀加时间有序标识
// 前缀标明实体类型，时间有序全局唯一标识提供时间有序性和全局唯一性
// 编号在实体创建时由语言运行时侧生成，同时直接作为数据库主键使用，不做自增编号和业务编号的翻译
// 选择时间有序全局唯一标识而非随机字符串，是因为其前半部分是毫秒时间戳，树形索引写入接近顺序追加
...
type Hypothesis struct {
	// 猜想编号，格式为猜想前缀加时间有序全局唯一标识，创建时生成，同时作为数据库主键
	// 后续任务和验证结果通过它建立关联
	ID string `json:"id"`
...
type Task struct {
	// 任务编号，格式为任务前缀加时间有序全局唯一标识，创建时生成，同时作为数据库主键
	// 证据通过它回连到具体取证动作
	ID string `json:"id"`
	// 所属运行编号；正式诊断管道必填，基线工具环可空（非诊断观察）
	// 空运行编号的任务与证据不得进入判决的证据引用链
	RunID string `json:"runId"`

	// 相关数据的编号，可以引用问题节点、确认目标或故障猜想
	Refs []string `json:"refs"`

	// 白名单工具名称，例如伪装列举容器组或集群列举容器组
	...
	// 当前最小闭环不使用，保留该字段是为了后续支持先查容器组名再查日志这类依赖链
...
type Evidence struct {
	// 证据编号，格式为证据前缀加时间有序全局唯一标识，创建时生成，同时作为数据库主键
	// 验证结果和报告通过它引用该证据
	ID string `json:"id"`
	// 所属运行编号，通常拷贝自任务；基线观察可空
	// 空值表示非诊断账本中的观察，不得作为正式判决的证据编号
	RunID string `json:"runId"`
	// 产生该证据的取证任务编号
	TaskID string `json:"taskId"`

	// 证据来源，例如伪装、集群、指标或日志系统
	Source string `json:"source,omitempty"`
	// 实际执行的工具名称，便于审计和复盘
	ToolName string `json:"toolName"`
	// 给用户看的等价查询表达，不代表系统执行了命令行外壳命令
...
// 每条判断都必须引用支撑它的证据编号
type Verdict struct {
	// 验证结果编号，格式为判决前缀加时间有序全局唯一标识，创建时生成，同时作为数据库主键
	// 报告通过它引用具体判断
	ID string `json:"id"`
...
type Report struct {
	// 报告编号，格式为报告前缀加时间有序全局唯一标识，创建时生成，同时作为数据库主键
	// 唯一标识一次运行的最终输出
	ID string `json:"id"`
	// 所属运行编号，用于存储层按运行查询报告
	RunID string `json:"runId"`
